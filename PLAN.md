...
intent TransferSafe(sender: Account, receiver: Account, amount: Int) {
  ...
  ensure sender.balance' == sender.balance - amount         // 后置条件（primed var = 新状态）
...
| 要素 | 语法 | 说明 |
|------|------|------|
| 类型定义 | `type Name { fields }` | 结构体/记录类型 |
| 枚举 | `enum Name { Variants }` | 代数数据类型 |
| 意图 | `intent Name(params) { ... }` | 核心构造，包含 require/ensure/invariant |
| 前置条件 | `require expr` | 调用意图前必须满足的条件 |
| 后置条件 | `ensure expr` | 意图执行后必须满足的条件 |
| 不变量 | `invariant expr` | 执行前后都必须满足的条件 |
| Primed 变量 | `x'` | 表示变量在 ensure/invariant 中的"新值" |
| 定理 | `theorem Name { expr }` | 需要被证明的性质 |
| 量词 | `forall x: T :: P(x)` / `exists x: T :: P(x)` | 全称/存在量词 |
| 蕴含 | `P ==> Q` | 逻辑蕴含 |
| 纯函数 | `function name(params) -> T { expr }` | 辅助定义，无副作用 |
