...
P2 两项启动前置解除后已逐条裁决，三项原方案全不采纳、批次结案（§6），
派生 D18/D19 入 P3；P3 未动
Created: 2026-08-10
...
- **不拆 references/**：语法速查是高频刚需而非低频参考，拆走会增加语法错误；
  ...
  （见附录 B）——P2 裁决时该存疑被证据坐实，D13 不做，见 §6。
...
~~**在拿到这对夹具之前，P2 不应启动。**~~ —— 夹具已取得（§10.2），P2 已据此
连同 §10.4 的证据完成裁决，见 §6。

---

## 6. P2 裁决（2026-08-10 结案：三项原方案全不采纳，D12 改走备选）

样本量为 n=1 时这三项缺乏第二个样本支撑，故推迟到 P1 在下一个真实项目跑通、
且 §5 的 eval 夹具就位后再判断。两项前置分别于 §10.2 与 §10.4 解除后，用取得的
证据逐条裁决如下——**三项的原方案全不采纳**，它们当初记下的「存疑点」在证据面前
都成立了。D11 与 D13 是纯粹的不做；D12 的原方案（合入主 CLI）被否，但它要解决的
分发诉求改由备选方案 D19 承担。本批次就此结案，不再悬置。

需要说明这不是「跳过 P2」：前置解除的意义是「现在可以裁决它们」，不是「现在
该建它们」。P2 从一开始就是待裁决清单而非待办清单。

### D11 facts.md 刚性机读子集 + `facts_schema: 1` —— 不做

原存疑点：schema 尚未稳定就先版本化是 YAGNI；D6 的解析计数自证可能已经够用。

**证据支持后者。** 对 `physics-sim.facts.md`（477 行 / 37 条事实）实测扰动：

| 情形 | 解析计数 | 诊断 | 退出码 |
|------|---------|------|--------|
| 原文件 | 37（30 BEH / 5 SUS / 2 UNK），31 confirmed | 无 | 0 |
| 人工独立清点 | 37（30 / 5 / 2），31 confirmed / 2 deferred / 4 rejected | —— | —— |
| `status:confirmed`（少一个空格） | 37，31 confirmed | 无 | 0 |
| fact_id 改小写 | 36，30 confirmed | ⚠️ 格式不匹配（带行号）+ ❌ .intent 引用的 id 找不到 | 1 |
| 删掉整行 `status:` | 37，30 confirmed + 1 unknown | ⚠️ status `(missing)`（带行号）+ ❌ 被引用但未 confirmed | 1 |

自报计数与人工清点逐项吻合。真正要防的「宽松解析静默漏项」没有发生：两种会
影响计数的扰动都带行号报出并以非零码失败，第三种被正确吸收、不丢数据。更关键
的是 **`.intent` 一侧会反向交叉核对**——凡 `.intent` 引用而 facts 文档里找不到
的 fact_id 一律报错。因此整体格式跑偏（例如把条目换成 Markdown 表格）会表现为
「解析到 0 条 + 全部引用缺失」，不可能悄悄通过。D6 的自证加这条交叉核对，已经
覆盖了 D11 想覆盖的全部风险。

唯一未覆盖的是空白差异被静默吸收，格式漂移得不到提示。这是数据无损的宽容，
不构成引入 schema 的理由。

**重新引入的触发条件**（记录以免将来重复论证）：当格式需要一次**不兼容**变更，
且存在本仓库之外的 facts.md 生产者时，再引入版本位。目前两个条件都不成立。

### D12 visualizer 合入主 CLI 成 `intent viz` —— 不合入，改为随 release 打包

原存疑点：便利性优化而非问题修复；让验证器吞下 `html_generator` 代价不小。

**证据把代价量化了，并暴露出一个比体量更根本的冲突。**

| | 代码行 | 文件数 | 内部依赖 |
|---|---|---|---|
| `tools/visualizer` | 2909 | 12 | 仅 `intent-lang-syntax` |
| `crates/intent-lang-cli` | 1521 | 2 | `intent-lang-core`（vendored z3，需 cmake）+ `intent-lang-accept` |

合入会让 CLI 代码量增至约三倍（`html_generator` 现为 746 行），但真正的问题是
依赖方向：**visualizer 现在完全不依赖 core，因而不依赖 z3**。§10.1 记录的架构
决策正是为此——推导层放 syntax 而非 core，好让「只想看图」的用户不必承担验证器
的构建成本。把 viz 塞进主 CLI 会直接推翻这个决策，让每个看图的人都去编译 z3，
恰好加重 V1 抱怨的那个分发问题。

V1 的分发诉求仍然成立，正确解法是 §8 当初判为「不够彻底」的那个备选：**把
visualizer 二进制纳入 release 多平台打包**（记为 **D19**，归入 P3）。它保住无 z3
的依赖图，而合入方案会毁掉它。此处推翻 §8 的原倾向，理由是写 §8 时 §10.1 的架构
决策尚不存在。

V5（viz 输出未版本化，落 `viz/<git-sha>/`）与合入与否无关，可在 visualizer 内部
独立实现，记为 **D18**，同归 P3。

### D13 write-intent 语法速查/反模式拆入 `references/` —— 不做

原存疑点：若 agent 不读中段散文，凭什么相信它会读另一个文件。

**§10.4 的证据直接反驳了「中段散文没被读」这个前提。** 该文件 351 行，分布：
...
| 语法速查 + 语法模板 | 87 | 24.8% |
| 工作流 | 40 | 11.4% |
| 输入是 facts.md 时 | 31 | 8.8% |
| 交付标准（DoD） | 25 | 7.1% |
| 质量规则 | 21 | 6.0% |
| 边界 | 8 | 2.3% |

§10.4 点名「被直接避免返工」的三条里，有一条正是**反模式 3**——据此把
`paused: bool` 判定为真正独立的开关而非生命周期，避免建出一条无终态的
Running↔Paused 环。占比最大的那一段恰恰是被读到并起了作用的那一段，拆走有害
无益。

语法速查（13.4%）则是提交 `327d582` 刻意内联进来的，目的是让技能可以装到别的
项目、服务于只有编译好的 `intent` CLI 的用户；拆入 `references/` 等于回退那次
决定。

351 行也谈不上臃肿：extract-facts 222 行（其中 46.4% 是必须内联的产物模板），
implement-testspec 106 行。三个技能目录目前都是单个 `SKILL.md` 平铺，无
`references/`，保持现状。
...
| D18 | visualizer 输出落 `viz/<git-sha>/` 而非固定路径，使多次生成可区分与归档 | V5（§6 裁决 D12 后从 P2 降入） |
| D19 | visualizer 二进制纳入 release 多平台打包，替代 D12 的合入方案解决分发 | V1（同上） |
...
| ~~visualizer 独立二进制随 release 多平台打包（V1 备选）~~ | ~~若要做则 D12 的合入方案更彻底；两者都在 P2~~ —— **§6 已推翻**：合入会让 viz 用户被迫编译 z3，违反 §10.1 的架构决策。打包这个备选反而是正解，已立为 D19 |
...
- **facts 格式**：D7 的 `deferred` 是 `status` 的新增取值，旧文档不含该值，
  不受影响。刚性 schema 已裁决不做（§6 D11），格式契约继续由 D6 的解析计数
  自证加 `.intent` 侧的 fact_id 交叉核对承担。
...
§5 eval 夹具（已取得，见 §10.2）──┐
                                  ├─→ P2 裁决（§6）：D11/D13 不做；
§10.4 第二个真实项目 telemetry ───┘    D12 原方案否决，派生 D18/D19 入 P3

P3（D14–D19）彼此独立，随时可做
```
...
- ~~**P2 前置之二**：在第二个真实项目上跑一遍 P1 全流程，取得 telemetry 基线~~
  —— 已完成，见 §10.4。两项前置解除后 P2 已逐条裁决、**批次结案**：D11 与 D13
  不做，D12 的合入方案否决（会让看图的人被迫编译 z3，违反 §10.1），理由与证据
  见 §6。由此派生 D18（viz 输出落 `viz/<git-sha>/`）与 D19（visualizer 纳入
  release 多平台打包，替代合入解决 V1 的分发诉求），均归 P3。
...
| V1 二进制未分发 | D1 消解其门槛诉求；分发由 D19 的 release 打包解决（§6 否决 D12 合入） | P1 / P3 |
| V2 只展示 dominant enum | D3 | P1 |
| V3 coverage「Covered: 0」 | D14 | P3 |
| V4 flowchart 缺无迁移 intent | D15 | P3 |
| V5 viz 输出未版本化 | D18（与合入解耦，在 visualizer 内部实现，见 §6） | P3 |
| C1 E0007 摩擦 | D16 | P3 |
| C2 E0006 摩擦 | D16 | P3 |
...
4. **`facts_schema: 1` 版本契约 → 降入 P2，代之以 D6 的解析计数自证。**
   ...
   地覆盖了「宽松解析静默漏项」这一顾虑。——P2 裁决时此判断被扰动实测坐实，
   D11 不做，见 §6。
